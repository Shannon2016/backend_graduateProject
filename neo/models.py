...
class Neo4j():
    ...
    def getEntityRelationbyEntity(self, value):
        answer = self.graph.run(
            "MATCH (entity1:Keyword) - [rel] -> (entity2:Title)  WHERE entity1.name = \"" + str(value) + "\" RETURN rel,entity2").data()
        return answer

    # 查找entity1及其对应的关系（与getEntityRelationbyEntity的差别就是返回值不一样）
    def findRelationByEntity(self, entity1):
        answer = self.graph.run(
            "MATCH (n1:LoreEntity1 {title:\"" + str(entity1) + "\"})- [rel] -> (n2:LoreEntity2) RETURN n1,rel,n2").data()
        # if(answer is None):
    ...
    def findRelationByEntity2(self, entity1):
        answer = self.graph.run(
            "MATCH (n1:LoreEntity2 {title:\"" + str(entity1) + "\"})- [rel] -> (n2:LoreEntity1) RETURN n1,rel,n2").data()
    ...
    def findOtherEntities(self, entity, relation):
        answer = self.graph.run(
            "MATCH (n1:LoreEntity1 {title:\"" + str(entity) + "\"})- [rel {type:\"" + str(relation) + "\"}] -> (n2:LoreEntity2) RETURN n1,rel,n2").data()
        # if(answer is None):
    ...
    def findRelationByEntities(self, entity1, entity2):
        answer = self.graph.run("MATCH (p1:LoreEntity1 {title:\"" + str(entity1) + "\"}),(p2:LoreEntity2{title:\"" + str(
        ...
        if(answer is None):
            answer = self.graph.run("MATCH (p1:LoreEntity1 {title:\"" + str(entity1) + "\"}),(p2:NewNode {title:\"" + str(
